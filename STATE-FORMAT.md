...
| 7 | 8 | `demod` carries `equaliserMode` and `equaliserConvergenceFactor` (`REQ-DEM-051`): whether the equaliser adapts, is frozen or is a unit impulse, and the LMS step size. A version 7 file has neither. Its equaliser always adapted, which is the default mode, and it offered no LMS mode for a step size to belong to, so the migration transforms nothing. The coefficients themselves are not in the file at either version — they are the result of measurements taken, not a setting, and a recalled setup should not restore an equaliser fitted to a channel that is no longer connected. |
